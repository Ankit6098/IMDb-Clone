const movieMoreInfoContainer = document.querySelector(
  ".movie-more-info-container"
);
const navContainer = document.querySelector(".nav-container");
const watchlistCardContainer = document.querySelector(
  ".watchlist-card-container"
);
const favouritelistCardContainer = document.querySelector(
...
async function fetchMovies() {
  moviesCardContainer.innerHTML = ""; // Clear previous content before API call
  ...
  if (data.Response === "False") {
    moviesCardContainer.innerHTML = `
          <div class="no-movie-found">
              <img src="https://cdn-icons-png.flaticon.com/512/7465/7465691.png" alt="No movie found">
          </div>
      `;
  } else {
  ...
  fetch(`https://www.omdbapi.com/?i=${imdbID}&apikey=72b8c020`)
    ...
    .then((data) => {
      ...
      if (watchlistLocalStorage.includes(data.imdbID)) {
        document.querySelector(".add-to-watchlist-button").innerHTML =
      ...
      if (favouriteListLocalStorage.includes(data.imdbID)) {
        document.querySelector(".add-to-favorites-button").innerHTML =
...
let watchlistLocalStorage = JSON.parse(
  localStorage.getItem("watchlistLocalStorage")
);
...
function addMovieToWatchlist(movieId) {
  ...
  if (movieMoreInfoContainer.style.opacity == "1") {
    const movieMoreInfoWatchlistButton = document.getElementById(
      `_watchlist-button-${movieId}`
    );
    if (watchlistLocalStorage.includes(movieId)) {
  ...
  if (watchlistIcon.classList.contains("active")) {
    ...
    watchlistIcon.src =
      "https://cdn-icons-png.flaticon.com/512/4485/4485491.png";
  }
  ...
  if (index !== -1) {
    ...
    localStorage.setItem(
      "watchlistLocalStorage",
      JSON.stringify(watchlistLocalStorage)
    );
    showWatchlist();
    showFavouriteList();
    return;
...
function showWatchlist() {
  ...
  if (watchlistLocalStorage.length == "") {
    ...
    for (let i = 0; i < watchlistLocalStorage.length; i++) {
      fetch(
        `https://www.omdbapi.com/?i=${watchlistLocalStorage[i]}&apikey=72b8c020`
      )
        .then((response) => response.json())
        .then((data) => {
          // movie ratings
...
let favouriteListLocalStorage = JSON.parse(
  localStorage.getItem("favouriteListLocalStorage")
);
...
function addMovieToFavouriteList(movieId) {
  ...
  if (movieMoreInfoContainer.style.opacity == "1") {
    const movieMoreInfoFavoritelistButton = document.getElementById(
      `_favoriteslist-button-${movieId}`
    );
    if (favouriteListLocalStorage.includes(movieId)) {
  ...
  const favouritelistIcon = document.getElementById(
    `_favouritelistIcon-${movieId}`
  );
  ...
  if (index !== -1) {
    ...
    localStorage.setItem(
      "favouriteListLocalStorage",
      JSON.stringify(favouriteListLocalStorage)
    );
    showFavouriteList();
    showWatchlist();
    return;
...
function showFavouriteList() {
  ...
  if (favouriteListLocalStorage.length == "") {
    ...
    for (let i = 0; i < favouriteListLocalStorage.length; i++) {
      fetch(
        `https://www.omdbapi.com/?i=${favouriteListLocalStorage[i]}&apikey=72b8c020`
      )
        .then((response) => response.json())
        .then((data) => {
          // movie ratings
...
function renderCard(element, data) {
  if (
    watchlistLocalStorage.includes(data.imdbID) &&
    favouriteListLocalStorage.includes(data.imdbID)
  ) {
    if (data.Poster == "N/A") {
      element.innerHTML = `
